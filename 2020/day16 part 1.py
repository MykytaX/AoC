...
def parser(line, total=0, number=0, operables=[], operflag = 0):
    while number != 0 or total != 0:
        ...
        if newpacket.literal == True:
            if total > 0:
                    ...
                    newpacket.operables.append(newpacket.literalvalue)
                    ...
                    if total > 0:
                        ...
                        return line, newpacket.operables
            if number > 0:
                    newpacket.literalvalue, leftovers = literalvalueparser(line[6:])
                    operables.append(newpacket.literalvalue)
                    line = leftovers
                    number = number - 1
                    if number > 0:
                        operflag = 1
        ...
        if newpacket.operator == True:
            ...
            if newpacket.lengthtypeid == 0:
                ...
                if newpacket.typeid == 0:
                    newpacket.value = sum(operables)
                newpacket.operables.append(newpacket.value)
                if total > 0:
                    total = total - (len(line)-len(leftovers))
                if number > 0:
                    number = number - 1
                ...
                leftovers, operables = parser(newpacket.contents, 0, newpacket.numberofsubs)
                if newpacket.typeid == 0:
                    newpacket.value = sum(operables)
                newpacket.operables.append(newpacket.value)
                if total > 0:
                ...
                line = leftovers
    operflag = 0
    return line, newpacket.value

parser(hexparser(example1),0,1)
print(line)
